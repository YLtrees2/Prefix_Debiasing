...
#prefix_prompts = load_word_list("data/w_prompts_selected.txt")



def read_data(input_file, prefix_prompts, n_subsamples, bias_tested):
    """
    ...
    with open(input_file) as f:
        ...
        for row in reader:
            ...
            if bias_type != bias_tested:
                continue


            #prefix = np.random.choice(prefix_prompts, 1)[0]
            prefixes = np.random.choice(prefix_prompts, n_subsamples)
            for prefix in prefixes:


                sent1, sent2 = '', ''
                if direction == 'stereo':
                    sent1 = prefix+row['sent_more']
                    sent2 = prefix+row['sent_less']
                else:
                    sent1 = prefix+row['sent_less']
                    sent2 = prefix+row['sent_more']
...
def evaluate(args):
    ...
    prefix_prompts = load_word_list(args.prefix_file)
    n_subsamples = int(args.n)
    df_data = read_data(args.input_file, prefix_prompts, n_subsamples, args.bias_tested)
...
parser.add_argument("--prefix_file", type=str, help="path to generated prefix prompts file")
parser.add_argument("--bias_tested", type=str, help="type of bias being tested")
parser.add_argument("-n", type=int, help="number of subsamples")
