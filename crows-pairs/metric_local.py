...
def evaluate(args):
    ...
    print("Model or Path:", args.model_or_path)
    print("=" * 100)
    ...
    if args.lm_model == "bert":
        tokenizer = BertTokenizer.from_pretrained(args.model_or_path)
        model = BertForMaskedLM.from_pretrained(args.model_or_path)
        uncased = True
    elif args.lm_model == "roberta":
        tokenizer = RobertaTokenizer.from_pretrained(args.model_or_path)
        model = RobertaForMaskedLM.from_pretrained(args.model_or_path)
        uncased = False
    elif args.lm_model == "albert":
        tokenizer = AlbertTokenizer.from_pretrained(args.model_or_path)
        model = AlbertForMaskedLM.from_pretrained(args.model_or_path)
        uncased = True
...
parser.add_argument("--model_or_path", type=str, help="model name or path to local model")
